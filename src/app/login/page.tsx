...
function LoginForm() {
  ...
  const defaultRole = searchParams.get('role') || 'broker';
  const [role, setRole] = useState<'admin' | 'broker'>(defaultRole as any);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(role === 'admin' ? '/admin' : '/broker');
  };
            ...
            {(['broker', 'admin'] as const).map((r) => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                  role === r ? 'bg-amber-500 text-slate-900' : 'text-gray-400 hover:text-white'
                }`}
              >
                {r === 'admin' ? '🛡️ Admin' : '🤝 Broker'}
              </button>
            ))}
          </div>
